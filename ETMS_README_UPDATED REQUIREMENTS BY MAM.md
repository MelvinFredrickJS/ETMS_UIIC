...
| 4 | Complaint flow | Required manager approval | **Auto-assigned** to Infra Team directly |
...
### TYPE 1: Complaint 🔴
> **Auto-assigned directly to Infra Team. No manager approval needed.**
...
**Cron job file:** `server/jobs/slaEscalationJob.js`
```js
...
//   AND escalated = FALSE
// For each: set escalated=TRUE, escalated_at=NOW(), log, email manager
```
...
| View all tickets | ❌ | Category queue | ✅ | Own only |
| Manage users | ❌ | ❌ | ✅ | ❌ |
| Create assets | ❌ | ❌ | ✅ | ❌ |
| Transfer assets | ❌ | ✅ (domain) | ✅ | ❌ |
...
│   │   │   ├── axiosInstance.js
│   │   │   ├── authApi.js
│   │   │   └── ticketApi.js         # all ticket, asset, user, approval APIs
│   │   ├── components/
│   │   │   ├── admin/
│   │   │   │   └── CategoryOrgGraph.jsx  # SVG graph: managers → teams → employees
│   │   │   ├── common/
│   │   │   │   ├── StatusBadge.jsx
│   │   │   │   ├── PriorityBadge.jsx
│   │   │   │   └── TypeBadge.jsx
...
│   │   ├── constants/
│   │   │   ├── ROLES.js              # employee | manager | admin | data_team
│   │   │   ├── TICKET_STATUS.js
│   │   │   ├── PRIORITY.js
│   │   │   └── TICKET_TYPES.js       # complaint | request | data
│   │   ├── context/
│   │   │   └── AuthContext.jsx
│   │   ├── pages/
│   │   │   ├── LoginPage.jsx          # shared — detects data_team → redirect /data-portal
│   │   │   ├── ChangePasswordPage.jsx
│   │   │   ├── DashboardPage.jsx      # employee / manager / admin dashboard
│   │   │   ├── TicketsPage.jsx
│   │   │   ├── NewTicketPage.jsx      # 3-step wizard + SLA days field + Report Issue pre-fill
│   │   │   ├── TicketDetailPage.jsx
│   │   │   ├── PendingApprovalsPage.jsx  # manager only — REQUEST queue
│   │   │   ├── AssetsPage.jsx         # employee: cards | infra team: full table
│   │   │   ├── AdminPage.jsx          # 3 tabs: Users | Assets | All Tickets
│   │   │   └── DataPortalPage.jsx     # data_team role only — separate portal
│   │   ├── App.jsx
│   │   └── main.jsx
│   └── package.json
│
...
│   │   ├── userController.js
│   │   ├── reportController.js
│   │   └── dataPortalController.js    # data team ticket raise + status view
│   ├── jobs/
│   │   └── slaEscalationJob.js        # node-cron daily SLA checker + escalation emailer
│   ├── middleware/
│   │   ├── authMiddleware.js
│   │   ├── roleMiddleware.js
│   │   └── uploadMiddleware.js
...
│   │   ├── userRoutes.js
│   │   ├── reportRoutes.js
│   │   └── dataPortalRoutes.js        # /api/data-portal/*
│   ├── services/
│   │   ├── assignmentService.js       # round-robin to team
│   │   └── emailService.js            # 8 email triggers
│   ├── utils/
│   │   ├── generateTicketId.js
│   │   ├── jwtUtils.js
│   │   ├── sanitizeFilename.js
...
| POST | /api/approvals/:id/approve | Manager (domain only) |
| POST | /api/approvals/:id/reject | Manager (domain only) |
| POST | /api/approvals/:id/reapprove | Manager (domain only) |
...
| PATCH | /api/assets/:id/status | Admin, Manager, Infra Team |
...
CREATE TABLE users (
  ...
  category_id         INTEGER,          -- FK ticket_categories.id (added after)
...
-- 2. ticket_types  (3 rows seeded: complaint | request | data)
...
-- 3. ticket_categories
-- Each category maps to: one ticket type, one manager, one team (via category_key),
-- and a requires_approval flag.
CREATE TABLE ticket_categories (
  ...
  manager_user_id   INTEGER REFERENCES users(id), -- NULL for auto-routed categories
  assigned_team_key VARCHAR(50) NOT NULL,          -- which team gets auto-assigned tickets
  requires_approval BOOLEAN DEFAULT FALSE          -- TRUE only for REQUEST categories
);
...
-- 5. asset_assignments (transfer history)
...
CREATE UNIQUE INDEX uq_asset_active ON asset_assignments(asset_id) WHERE returned_at IS NULL;

-- 6. tickets  ← includes SLA columns
CREATE TABLE tickets (
  ...
  status            VARCHAR(30) NOT NULL DEFAULT 'pending_approval'
                    CHECK (status IN (
                      ...
                      'in_progress','reported','resolved','closed','rejected'
                    )),
  raised_by         INTEGER NOT NULL REFERENCES users(id),
  assigned_to       INTEGER REFERENCES users(id),
  approval_owner_id INTEGER REFERENCES users(id),  -- NULL for auto-routed tickets
...
CREATE TABLE ticket_logs (
  ...
  -- actions: TICKET_CREATED | APPROVED | REJECTED | AUTO_ASSIGNED | MANUALLY_ASSIGNED
  --          STATUS_CHANGED | ESCALATED | SLA_ESCALATED | BACK_TO_MANAGER | RE_APPROVED
...
-- ── Users ──
-- Hash = Password@123 : $2b$10$NW3HlIvAopV24VW.BRDZ9.Vgg1CG1.vlJDk9Mtq.iMoHkmkCLVKZi
INSERT INTO users (emp_id, name, email, password_hash, role, department, password_changed_at) VALUES
  -- Admin
  ('EMP001', 'Admin User',      'admin@uiic.co.in',          '...hash...', 'admin',     'IT',      NOW()),
  -- Managers (2 only)
  ('MGR-INFRA', 'Infra Manager',   'mgr.infra@uiic.co.in',   '...hash...', 'manager',   'IT',      NULL),
  ('MGR-NET',   'Network Manager', 'mgr.network@uiic.co.in', '...hash...', 'manager',   'Network', NULL),
  -- Sample employees (one per team — expand as needed)
  ('T-EMAIL-01', 'Email Team Member 1',   'email1@uiic.co.in',   '...hash...', 'employee', 'IT',       NULL),
  ('T-VC-01',    'VC Team Member 1',      'vc1@uiic.co.in',      '...hash...', 'employee', 'IT',       NULL),
  ('T-INFRA-01', 'Infra Team Member 1',   'infra1@uiic.co.in',   '...hash...', 'employee', 'IT',       NULL),
  ('T-NET-01',   'Network Team Member 1', 'net1@uiic.co.in',     '...hash...', 'employee', 'Network',  NULL),
  ('T-SEC-01',   'Security Team Member 1','sec1@uiic.co.in',     '...hash...', 'employee', 'Security', NULL),
  ('T-SAP-01',   'SAP Team Member 1',     'sap1@uiic.co.in',     '...hash...', 'employee', 'SAP',      NULL),
  ('T-GC-01',    'GC Master Member 1',    'gc1@uiic.co.in',      '...hash...', 'employee', 'GC',       NULL),
  ('T-RPT-01',   'Reports Team Member 1', 'reports1@uiic.co.in', '...hash...', 'employee', 'Reports',  NULL),
  -- Data team
  ('DATA-01', 'Data Team Member 1', 'data1@uiic.co.in', '...hash...', 'data_team', 'Data', NULL);

-- ── Ticket Categories ──
-- REQUEST categories → requires_approval = TRUE, manager_user_id set
-- COMPLAINT categories → requires_approval = FALSE, manager_user_id = NULL, assigned_team_key set
-- DATA categories → requires_approval = FALSE, manager_user_id = NULL, assigned to security_team

-- Complaint categories (auto-route, no approval)
INSERT INTO ticket_categories
  (ticket_type_id, name, category_key, default_priority, manager_user_id, assigned_team_key, requires_approval)
VALUES
  (1,'Hardware Issue',   'hardware_complaint', 'high',   NULL, 'infra_team',    FALSE),
  (1,'Network Issue',    'network_complaint',  'high',   NULL, 'network_team',  FALSE),
  (1,'Email Issue',      'email_complaint',    'medium', NULL, 'email_team',    FALSE),
  (1,'VC Issue',         'vc_complaint',       'medium', NULL, 'vc_team',       FALSE),
  (1,'SAP Issue',        'sap_complaint',      'medium', NULL, 'sap_team',      FALSE),
  (1,'GC Master Issue',  'gc_complaint',       'medium', NULL, 'gc_master_team',FALSE),
  (1,'Reports Issue',    'reports_complaint',  'medium', NULL, 'reports_team',  FALSE);

-- Request categories — Infra Manager
INSERT INTO ticket_categories
  (ticket_type_id, name, category_key, default_priority, manager_user_id, assigned_team_key, requires_approval)
VALUES
  (2,'Credential Request',  'credential_request', 'medium', (SELECT id FROM users WHERE emp_id='MGR-INFRA'), 'infra_team',   TRUE),
  (2,'New Hardware',        'new_hardware',        'low',    (SELECT id FROM users WHERE emp_id='MGR-INFRA'), 'infra_team',   TRUE),
  (2,'New Software',        'new_software',        'low',    (SELECT id FROM users WHERE emp_id='MGR-INFRA'), 'infra_team',   TRUE);

-- Request categories — Network Manager
INSERT INTO ticket_categories
  (ticket_type_id, name, category_key, default_priority, manager_user_id, assigned_team_key, requires_approval)
VALUES
  (2,'Port Request',  'port_request',  'high',   (SELECT id FROM users WHERE emp_id='MGR-NET'), 'network_team',  TRUE),
  (2,'SR Request',    'sr_request',    'medium', (SELECT id FROM users WHERE emp_id='MGR-NET'), 'network_team',  TRUE),
  (2,'Gate Pass',     'gate_pass',     'low',    (SELECT id FROM users WHERE emp_id='MGR-NET'), 'security_team', TRUE),
  (2,'ADID Request',  'adid_request',  'medium', (SELECT id FROM users WHERE emp_id='MGR-NET'), 'network_team',  TRUE);

-- Data categories (auto-route to security team)
INSERT INTO ticket_categories
  (ticket_type_id, name, category_key, default_priority, manager_user_id, assigned_team_key, requires_approval)
VALUES
  (3,'Data Audit Request',   'data_audit',    'medium', NULL, 'security_team', FALSE),
  (3,'Data Backup Request',  'data_backup',   'medium', NULL, 'security_team', FALSE),
  (3,'Paycheque Data',       'paycheque_data','medium', NULL, 'security_team', FALSE);

-- ── Assign employees to their teams via category_id ──
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='email_complaint')
  WHERE emp_id = 'T-EMAIL-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='vc_complaint')
  WHERE emp_id = 'T-VC-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='hardware_complaint')
  WHERE emp_id = 'T-INFRA-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='network_complaint')
  WHERE emp_id = 'T-NET-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='data_audit')
  WHERE emp_id = 'T-SEC-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='sap_complaint')
  WHERE emp_id = 'T-SAP-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='gc_complaint')
  WHERE emp_id = 'T-GC-01';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='reports_complaint')
  WHERE emp_id = 'T-RPT-01';
-- Managers assigned to their primary category
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='hardware_complaint')
  WHERE emp_id = 'MGR-INFRA';
UPDATE users SET category_id = (SELECT id FROM ticket_categories WHERE category_key='network_complaint')
  WHERE emp_id = 'MGR-NET';
```
...
| Data Team | data1@uiic.co.in | Password@123 | /data-portal |
...
3. Status transition logic lives **only** in `utils/ticketTransitions.js`
4. Round-robin assignment logic lives **only** in `services/assignmentService.js`
5. SLA escalation logic lives **only** in `jobs/slaEscalationJob.js`
...
13. File naming uses `sanitizeFilename.js` — never raw `originalname`
