...
│   │   │   └── ticketApi.ts        # All ticket, asset, user, approval, lookup APIs
│   │   ├── components/
│   │   │   ├── admin/
...
│   │   │   ├── DashboardPage.tsx
│   │   │   ├── TicketsPage.tsx
│   │   │   ├── NewTicketPage.tsx    # Supports pre-fill from AssetsPage (Report Issue)
│   │   │   ├── TicketDetailPage.tsx
│   │   │   ├── PendingApprovalsPage.tsx
│   │   │   ├── AssetsPage.tsx       # Employee: cards + Report Issue | Manager: full table
...
│   └── fix_categories.sql           # Migration: replaces custom categories with spec categories
│
...
| Employee | (any employee email)     | Password@123 |
...
- Status-coloured ticket cards (8 distinct colours per status)
...
- All assets mapped to `hardware_issue` category (hardware team manages them)
- **Report Issue button** on each asset card/row → pre-fills New Ticket page with:
  - Type: Complaint, Category: Hardware Issue, Asset: pre-selected
  - User skips directly to Step 3 (Details)
- Admin: 3-tab panel (Users / Assets / All Tickets)
- Manager: category-scoped asset table with Transfer, Status, History, Report Issue
- Employee: asset cards with Report Issue button (active assets only)
- Assignment history drawer per asset
- Round-robin auto-assignment when creating a new employee (least-recently-assigned asset)

### Admin Panel
- **Users tab** — add/delete users, transfer ownership, employee lookup by Emp ID or serial number, Category Org Graph (SVG visualisation of manager → employee relationships per type)
...
### Category Org Graph (Admin only)
- SVG graph inside Users tab showing manager → employee hierarchy per category
- Filter by ticket type: All / Complaints / Requests / Data
- Purple nodes = managers, teal nodes = employees, teal edges = assignment
- Hover tooltip: emp_id, full name, category, approval mode
- Click node to highlight that category, dim others

### Employee Lookup (Admin only)
...
9. Department column from Excel is intentionally not mapped to categories
